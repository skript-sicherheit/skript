...
\end{align*}
Wenn Alice uns eine Nachricht $M \in \mathbbm{G}$ schicken möchte, wählt sie analog zu $x$ ein $y$ zufällig gleichverteilt, berechnet damit $Y = g^y \in \mathbbm{G}$ und $C = h^y M \in \mathbbm{G}$ und sendet das Tupel $(Y ,C)$. Wir können die Nachricht entschlüsseln, indem wir auflösen:
\begin{align*}
C &= h^y M\\
\Leftrightarrow \quad M& = \frac{C}{h^y}
 = \frac{C}{g^{xy}}
 = \frac{C}{(g^y)^x}
 = \frac{C}{Y^x}
\end{align*}
