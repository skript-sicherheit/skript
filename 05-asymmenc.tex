...
\begin{itemize}
  ...
  \item wähle $e$ mit $\text{ggT}(e, (P-1)(Q-1)) = 1$
...
\begin{description}
    \item[Wahl von $e$:] Aus Effizienzgründen liegt auf den ersten Blick es nahe, den Parameter $e$ aus dem öffentlichen Schlüssel nicht für jeden Benutzer neu
    ...
    führt jedoch zu Problemen, wenn dieselbe Nachricht $M$ an mindestens drei unterschiedliche Benutzer verschlüsselt wird. Setzen wir für dieses Beispiel $e =
    ...
    öffentlichen Schlüsseln $(e_1, N)$ und $(e_2, N)$ chiffriert und gilt weiterhin $\text{ggT}(e_1, e_2) = 1$ in $\Z N$, kann ein Angreifer aus den Chiffraten
    $M$ berechnen:
    \begin{align*}
    re_1 + se_2 & = 1\\ 
    \Longrightarrow C_1C_2 \mod N & \equiv M^{re_1}M^{se_2} \mod N\\
...
Entschlüsselungsfunktion vereinfachen die Implementierung zusätzlich. Mit einfachen Anpassungen ($e = 3$ bei Verschlüsselung, CRS nutzen bei Entschlüsselung)
