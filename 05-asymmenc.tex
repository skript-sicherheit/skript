...
\begin{definition}[IND-CPA-Sicherheit für Public-Key-Verfahren]
  ...
  PPT-Angreifer $\A$.
  \begin{itemize}
  \item $\C$ generiert mit dem Generator-Algorithmus ein
  Schlüsselpaar, d.h. er berechnet $(pk, sk)$.
  \item $\A$ erhält $pk$ und kann sich zu jedem Zeitpunkt jedes beliebige $\plaint$
...
\subsection{Sicheres RSA}\label{ssec:sicheres-rsa} Wir haben festgestellt, dass RSA
...
= \frac{C}{(g^y)^x},
\end{align*}
wobei $y$ zufällig gewählt wird. Es gilt also 
\begin{align*}
  C &\equiv h^y M \\
...
\begin{definition}[DDH-Annahme]\label{def:ddh}
...
die Anzahl an Schlüsseln quadratisch in der Anzahl der Teilnehmer
wächst.
