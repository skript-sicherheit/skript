...
geschieht über eine sogenannte \textit{Public Key Infrastructure}. Dies
kann zum Beispiel dadurch sichergestellt werden, dass Alice und Bob über
einen weiteren Kanal den Hashwert des Schlüssels mittels einer
kollisionsresistenten Hashfunktion vergleichen.
\section{RSA}
...
Wir betrachten im Folgenden zwei Varianten des Euklidischen
Algorithmus.
...
\begin{itemize}
 	...
 	\item Berechne $\varphi(N) = (P - 1)(Q - 1)$\footnote{$\varphi$
            ...
            Die ist multiplikativ, d.h. es gilt für teilerfremde $n$, $m$:
            $\varphi(m\cdot n) = \varphi(m) \cdot \varphi(n)$. Da eine
            Primzahlen $p$ nur durch 1 nd sich selbst teilbar sind, sind
            gilt $\varphi(p) = p-1$.} 
...
Das bisher behandelte Verfahren nennt sich \textit{Textbook-RSA} und
umfasst das grundlegende Prinzip von RSA. Textbook-RSA weist einige
Schwächen auf, die im nächsten Kapitel genauer besprochen werden. Deshalb solltees in der Praxis nicht verwendet werden.
...
\begin{definition}[IND-CPA-Sicherheit für asymetrische Chiffren]
  ...
  \end{enumerate} Ein Verfahren heißt IND-CPA-sicher, wenn der Vorteil des
  Angreifers gegenüber dem Raten einer Lösung, also $\Pr \left[ \A
    \textnormal{ gewinnt} \right] - \frac{1}{2}$, für alle Angreifer $\A$
  vernachlässigbar ist.
\end{definition}

Der IND-CPA-Begriff unterscheidet sich also dadurch, dass ein Angreifer
...
verschlüsselt. Dadurch kann ein Angreifer zwei Chiffrate effizient voneinander
unterscheiden (z.B. $\enc(pk, \text{annehmen})$ und $\enc(pk,
\text{ablehnen})$). RSA ist also nicht IND-CPA-sicher. 

Es ist leicht zu sehen, dass RSA höchstens so schwer ist wie das
Faktorisieren großer Zahlen in Primzahlen. Wenn ein Angreifer in der
Lage ist, eine Primfaktorzerlegung von $N$ zu berechnen, erhält er $P$
und $Q$ und damit $\varphi(N)$. Damit kann er dann mit dem EEA und $e$
aus dem $pk$ $d = d^{-1} (mod \varphi(N))$ berechnen und hat damit dem
privaten Schlüssel.\footnote{Oft ließt man, dass RSA zu brechen genauso
  schwer ist wie die Primfaktorzerlegung großer Zahlen. Es ist aber
  nicht bekannt, ob es auch eine Möglichkeit gibt, den $sk$ zu
  berechnen, ohne eine Primfaktorzerlegung durchführen zu müssen.} 

Zur Zeit ist kein PPT-Algorithmus zum Faktorisieren bekannt.
Es gibt jedoch noch einige andere Angriffspunkte, die im Folgenden umrissen
werden. 
...
\item[Wahl von $e$:] Aus Effizienzgründen liegt es auf den ersten Blick
  ...
  dieselbe Nachricht $M$ an mindestens drei unterschiedliche Benutzer
  verschlüsselt gesendet wird. Ein Angreifer, der die drei öffentlichen
  ...
  \begin{align*}
    X \equiv M^e \mod N_1N_2N_3
  \end{align*}

Da $M^e<N_1N_2N_3$, kann man nun die $e$-te Wurzel von $X$ in $\mathbbm{Z}$
...
\item[Wahl von $N$:] 
      Bei Wahl eines festen $N$ ergeben sich zwei Angriffspunkte:
      \begin{itemize}
      \item Wird wieder dieselbe Nachricht $M$ mit zwei
    öffentlichen Schlüsseln $(N, e_1)$ und $(N, e_2)$ chiffriert und gilt weiterhin $\text{ggT}(e_1, e_2) = 1$ in $\Z{}$, kann ein Angreifer aus den Chiffraten
    $M$ berechnen:
    \begin{align*}
    re_1 + se_2 & = 1\\ 
    ...
    \end{align*}
  \item
    Ist $N$ fest gewählt, dann sind $P$ und $Q$ für alle Anwender
    gleich, also kennt ein Angreifer $\varphi(N)$. Damit kann er nun
    einfach ein $d'$ berechnen mit \[d'=e^{-1} \mod \varphi(N)\].
  \end{itemize}    
...
aus Grafik \ref{fig:rsa-oaep} an und verschlüsselt danach mit RSA:
\begin{itemize}
...
\begin{align*}
  ...
  Y & = R \oplus H(X)
\end{align*}
\item Erzeuge ein Schlüsselpaar $(pk, sk)$ mit dem
  RSA-Generator-Algorithmus
...
geeigneten Gruppe $G$ von entscheidendet Bedeutung. ElGamal ist genau
dann IND-CPA-sicher, wenn in $G$ die \emph{decisional
...
$\mathbbm{G}$ sind echte Untergruppen von $\mathbbm{Z}^*_p$ mit
$p$ prim und $|\mathbbm{G}| \approx 2^{2048}$.
...
modulo $N$, bei ElGamal die DDH-Annahme\footnote{Insbesondere fallen
  diese Annahmen, wenn $P=NP$. Dann könnte ein Angreifer einfach den
  $sk$ raten u damit entschlüsseln}.
