...
\begin{tabular}{lllll}
\cline{1-4}
\multicolumn{1}{|c|}{\begin{tabular}[c]{@{}c@{}}TLS Handshake\\Protocol\end{tabular}} &
\multicolumn{1}{c|}{\begin{tabular}[c]{@{}c@{}}TLS Change Cipher Spec\\Protocol\end{tabular}} &
\multicolumn{1}{c|}{\begin{tabular}[c]{@{}c@{}}TLS Alert\\Protocol\end{tabular}} &
\multicolumn{1}{c|}{\begin{tabular}[c]{@{}c@{}}TLS Application Data\\Protocol\end{tabular}} & \\ \cline{1-4} \multicolumn{4}{|c|}{TLS Record
Protocol} & \\ \cline{1-4} & & & & \\ & & & &
\end{tabular}
...
\item[Beobachtung:] Server und Client tauschen zu Beginn ihrer
  ...
  verschlüsselte/authentifizierte Kommunikation umzuschalten.
\item[Beobachtung 2:] \emph{ChangeCipherSpec} ist nicht Teil des MACs über den
  Handshake, und die Spezifikation verlangt nicht, dass \emph{ChangeCipherSpec}
  gesendet werden muss, bevor \emph{Finished} gesendet wird. 
\item[Beobachtung 3:] Es gibt \texttt{CipherSpec}s welche nur authentifizieren,
  ohne zu verschlüsseln.
\item[Angriff:] Ein aktiver Angreifer unterdrückt den
  \emph{ChangeCipherSpec}-Hinweis.
\item[Konsequenz:] Wird eine \texttt{CipherSpec} verwendet, welche nur authentifiziert,
  so senden beide Seiten nun Nachrichten im Klartext mit MAC, gehen aber davon aus,
  dass sie klartext Nachrichten \emph{ohne} MAC erhalten.
  Der Angreifer kann also nun den MAC entfernen und die Nachrichten beliebig verändern.
\item[Gegenmaßnahme:] Aktzeptiere \emph{Finished} Nachrichten nur, wenn zuvor
  eine \emph{ChangeCipherSpec} Nachricht kam.
\end{description}
