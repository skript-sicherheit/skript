...
formal geklärt.
%TODO aktiv sicher ist nirgentwo definiert. Moritz meint, es heißt IND-CCA.
...
\subsection{Diffie-Hellman-Schlüsselaustausch} \label{sec:ddh-key-exchange}
...
TLS (\emph{Transport Layer Security}) ist eine 1999 standadisierte
Weiterentwicklung des von Netscape entwickelten Protokolls SSL
(\emph{Secure Socket Layer}). Das Protokoll besteht aus 5
Teilprotokollen (siehe Tabelle \ref{tbl:tls}). Das Ziel von TSL ist es,
ein sicheres Verschlüsselungsverfahren für die Kommunikation ü
\\
\begin{table}[h]
\centering
\begin{tabular}{lllll}
...
TLS setzt auf die Transportschicht des OSI-Modell auf\footnote{Die
...
\begin{description}
	...
	\item[Angriff:] Eine vereinfachte Darstellung des zu übertragenden Schlüssels $K$ ist:
		...
		\begin{align*}
		M_i := \alpha_i \cdot (0\mathrm{x}0002 \parallel \mathtt{rnd} \parallel 0\mathrm{x}00 \parallel K) \mod N = \dec(\alpha_i^e \cdot C \mod
		N)
		\end{align*}
