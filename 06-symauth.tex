\chapter{Symmetrische Authentifikation von Nachrichten}\indexMessageAuthSymm
\label{cha:symauth}
...
eine "`Unterschrift"' $\sigma$ \indexSig und überträgt diese gemeinsam mit
...
\emph{Message Authentication Codes} (MACs) \indexMAC sind ein symmetrisches
...
\section{Der EUF-CMA-Sicherheitsbegriff}\indexEUFCMA
...
Ein MAC (\sig, \ver) ist \textit{EUF-CMA-sicher\footnote{\textit{EUF} \indexEUFCMA steht für
...
PPT-Algoritmus \A~das obige Spiel nur mit (im Sicherheitsparameter $k$)
...
beliebiger Länge signieren können. Hierzu bieten sich Hash-Funktionen \indexCryptHashFunction an, die
...
\begin{theorem} Sei $(\sig, \ver)$ EUF-CMA-sicher \indexEUFCMA und $H$ eine
...
\begin{definition}[Pseudorandomisierte Funktion (PRF)\indexPRF] Sei  \[\text{\textit{PRF}}\colon\{0,1\}^k \times \{0, 1\}^k \rightarrow
...
\begin{figure}[h]
  \begin{center} \unitlength=1mm \linethickness{0.4pt} \hspace{-3 cm}
    ...
    \end{picture}
  \end{center}
  \caption{\emph{Merkle-Damgård}-Konstruktion $H_{MD}$\indexMDTransformation. Es ist möglich, an
...
\begin{theorem}\indexPRF\indexCryptHashFunction Sei $\text{PRF}\colon\{0,1\}^k \times \{0,1\}^k
...
HMAC\indexHMAC. Das Signieren einer Nachricht funktioniert dabei wie folgt:
\begin{equation*}
