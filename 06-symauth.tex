...
\section{Symmetrische EUF-CMA-Sicherheit}
...
vorher an das Orakel gegeben zu haben. Etwas strukturierter sieht das
Experiment für einen PPT-Angreifer \A~so aus:

\begin{definition}
Betrachte folgendes Experiment mit einem Herausforderer \C~und einem
PPT-Algorithmus \A, bei dem \C~einen Schlüssel \key~zufällig gleichverteilt
wählt.   
\begin{enumerate}
...
%      }
      \textbf{\draw[->, thick] (0.3,-1) -- (6.7,-1.5)
        node[sloped,above,pos=0.5] {$M_i$};}
      ...
      \draw (-6,-6.5) node (b3) {$\wedge$};
      \draw (-5.8,-7) node (b4) {$M^* \notin \{M_1, \dots, M_n\}$?};
    \end{tikzpicture}
  }
