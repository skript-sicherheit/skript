...
Betrachten wir nun noch die Fehlerfortplanzung bei CTR. Wir stellen fest, dass wir gezielt Bits in verschiedenen Blöcken \(M_i\) verändern können, indem wir sie in den entsprechenden \(C_i\) verändern. Nachrichten verschlüsselt mittels CTR sind somit homomorph veränderbar. Wird hingegen der gewählte \(IV\) verändert, erfolgt eine komplette Zerstörung der ursprünglichen Nachricht. 
...
Im \textit{Galois/Counter Mode} (GCM) finden wir einen Modus mit obiger Eigenschaft inklusive den Vorteilen von CTR. Zusätzlich zum Verschlüsseln einer Nachricht \(M\), welches analog zu CTR erfolgt, wird gleichzeitig eine Signatur\footnote{Signaturen werden ausführlicher in \hyperref[cha6]{Kapitel 6} behandelt.} für die Nachricht generiert. Das Verschlüsseln von \(M\) im GCM liefert uns ein Chiffrat-Signatur-Paar (\(C, \sigma\)), wobei sich die Signatur \(\sigma\) unter anderem aus \(C\) ergibt\footnote{Für genauere Informationen verweisen wir auf die \href{http://csrc.nist.gov/groups/ST/toolkit/BCM/documents/proposedmodes/gcm/gcm-spec.pdf}{Veröffentlichung der Entwickler.}}. Die Signatur \(\sigma\) wird verwendet, um Manipulationen an \(C\) zu erkennen.
Vereinfacht dargestellt erhalten wir folgende Operationen:
\bigskip

%\begin{align*}
\begin{math}
	EncryptAndAuthenticate(M) \colon= (Encrypt((M), Authenticate(C)) = (C, \sigma) \\
	\\
	\indent DecryptAndAuthenticate(C, \sigma) \colon= \begin{cases}
...
Wir sehen, dass wir nur dann aus einem Chiffrat-Signatur-Paar (\(C, \sigma\)) die dazugehörige Nachricht \(M\) erhalten, wenn die Signatur \(\sigma'\) von \(C\) identisch mit der übergebenen Signatur \(\sigma\) ist. Wurde also ein Chiffrat \(C\) mit Signatur \(\sigma\) zu einem Chiffrat \(C'\) mit Signatur \(\sigma'\) verändert, schlägt die Entschlüsselung \(DecryptAndAuthenticate(C', \sigma)\) fehl, da die übergebene Signatur nicht mit der Signatur des Chiffrats übereinstimmt.
...
\centering
	\begin{tabularx}{\textwidth}{ | X | X | X | X | X |} 
		...
		\hline
		Synchronisation & TODO & TODO  & TODO & TODO \\ 
		\hline
		...
		\hline
	\end{tabularx}
	\caption{\(^{\ast}\) IND-CPA ist ein Sicherheitsbegriff und wird in \hyperref[IND-CPA]{Abschnitt \ref {IND-CPA}} definiert \\ 
...
Nachdem wir nun mehrere Betriebsmodi vorgestellt haben, betrachten wir jetzt Verschlüsselungsalgorithmen, die diese Modi verwenden können. 
