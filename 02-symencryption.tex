...
Die Tabellen sind zeilenweise zu lesen, so dass das durch den Zellenwert gegebene Bit auf das durch den Zellenindex, der in der ersten Tabellenspalte jeweils beispielhaft notiert ist, gegebene Bit abgebildet wird. Beispielsweise permutiert $P$ das achte Bit des Eingabestrings, also das Bit an Stelle 7, an die erste Position des Ausgabestrings, das heißt $F(R^{(i)}, \key^{(i)})[0]$ und das siebzehnte Bit an die letzte Stelle, das heißt $F(R^{(i)}, \key^{(i)})[31]$\footnote{Die Tabellen sind dem NIST-Standard \cite{NIST_DES99} entnommen und an die Schreibweise der Matsui-Veröffentlichung angepasst (Siehe S.~\pageref{matsui-notation}).}. 
...
\subsubsection{Authentifizerte Betriebsmodi}
...
Ein authentifzierter Betriebsmodus ist der \emph{Galois/Counter Mode} (GCM) \cite{NIST_GCM05}. Dieser verschlüsselt die Nachricht analog zum CTR-Modus und generiert die Signatur anhand einer Verknüpfung der einzelnen Chiffratblöcke. Vereinfacht dargestellt erhalten wir folgende Operationen, wobei $\ciphert \coloneqq \enc(\key_{1}, \plaint)$:
\begin{align*}
	EncryptAndAuthenticate(\key_{1}, \key_{2}, \plaint) &\coloneqq (\ciphert, \sig(\key_{2}, \ciphert)) = (\ciphert, \sigma)\\
	DecryptAndAuthenticate(\key_{1}, \key_{2}, \ciphert, \sigma) &\coloneqq \begin{cases}
	\dec(\key_{1}, \ciphert) & \ver(\key_{2}, \dec(\key_{1}, \ciphert), \sigma) = 1 \\
	\bot & \textnormal{sonst}
	\end{cases}
\end{align*}
Bei Bitfehlern unterscheiden wir zwischen Fehlern im Chiffrat- und im Signaturteil. Tritt ein Fehler im Chiffratteil auf, verhält sich GCM analog zu CTR: Wird das Bit $j$ im Block $\ciphert_i$ negiert, ist auch Bit $j$ in $\plaint_i$ negiert. Zusätzlich schlägt die Verifikation der Signatur fehl, da das Chiffrat nun nicht mehr zu $\plaint$ entschlüsselt. Bei einem Fehler im Signaturteil schlägt die Verifikation ebenso fehl, der Klartext bleibt hingegen unberührt. Jedoch verfügt der Empfänger über keine Möglichkeit mehr, die Integrität der Nachricht zu überprüfen.


%Ein Verfahren, dessen Signaturerzeugung zwar ebenfalls nicht parallelisierbar ist, insgesamt aber nur $n$-viele $\enc$-Operationen benötigt, ist der \emph{Galois/Counter Mode} (GCM) \cite{NIST_GCM05}. Im Unterschied zu CCM wird die Signatur anhand einer Verknüpfung der einzelnen Chiffratblöcke berechnet, während die Verschlüsselung der Nachricht analog im CTR-Modus stattfindet. Zusätzlich hat GCM den Vorteil, dass die verwendeten Operationen zur Signaturerzeugung deutlich effizienter implementiert werden können, als die entsprechende Blockchiffre \cite[S.~1]{NIST_GCM05}.
...
\begin{table}[h]
	...
	\begin{tabularx}{\textwidth}{ | >{\raggedright\arraybackslash}X | >{\raggedright\arraybackslash}X | >{\raggedright\arraybackslash}X | >{\raggedright\arraybackslash}X | >{\raggedright\arraybackslash}X |} 
		...
		& Bit $j$ im Block $\plaint_i$ negiert & Bit $j$ im Block $\plaint_i$ negiert und Signatur-Verifikation schlägt fehl\\
		\hline
	\end{tabularx}
