...
\begin{figure}[h]
	...
	\node (ENC) at (0,\EncYCoord) [draw,fill=black!15,rectangle,minimum width=1cm,minimum height=0.65cm] {$\enc$};
	...
	\end{scope}
	\end{tikzpicture}
	\caption{Prinzip einer Stromchiffre. Der Klartextstrom $(\plaint_1,\plaint_2,\dots,\plaint_n)$ wird zeichenweise mit einem, aus dem Schlüssel $\key$ mit Generator $G$ erzeugten Bitstrom $(b^{(1)}, b^{(2)}, \ldots, b^{(n)})$ durch $\enc$ verschlüsselt.}
...
Das besondere an einer Feistel-Struktur ist, dass sie, trotz Nichtinvertierbarkeit der Bausteine, selbst invertierbar ist.
