...
der zugehörige Klartextblock völlig zerstört, aber im nächsten Klartextblock wird genau dieses Bit negiert, was von entscheidender Bedeutung sein kann. Nach
...
Der Counter Mode ähnelt in seiner Vorgehensweise einer Stromchiffre. Zu Beginn wird ein Initialisierungsvektor $IV$ zufällig gewählt. Bei der Verschlüsselung wird ein Chiffratblock $\ciphert_i$ durch $\ciphert_i = \enc_\key(IV + i)\oplus\plaint_i$ erzeugt. Der Initialisierungsvektor wird also für jeden Block um eins inkrementiert, weshalb das Verfahren Counter Mode ("`Zählermodus"') heißt. Im Vergleich mit Stromchiffren können wir $\enc_\key(IV + i)$ als Generator betrachten.\\ \ \\
Wir können eine Nachricht entschlüsseln, indem wir $\ciphert_i$ und $\plaint_i$ tauschen: $\plaint_i = \enc_\key(IV + i)\oplus\ciphert_i$. Wir stellen fest, dass wir auf die Entschlüsselungsfunktion $\dec$ nicht angewiesen sind.\\ \ \\
Der CTR Modus hat im Wesentlichen die selben Eigenschaften wie der CBC Modus. Er lässt sich jedoch deutlich leichter parallelisieren. Leider ist auch der CTR Modus homomorph veränderbart, weshalb er nicht vor aktiven Angriffen schützt. Abhilfe schafft der Galois Counter Mode (GCM), der den CTR Modus um Prüfsummen erweitert.
