...
Zustand $K \in \{0,1\}^k$, der initial auf den Schlüsselwert gesetzt wird und eine Funktion $SC(K) \coloneqq (b, K') \in \{0,1\} \times \{0,1\}^k$, die die zur Verschlüsselung notwendigen Bits erzeugt. 

Formal definieren wir hierzu eine Funktion $G(K) \coloneqq (b^{(1)},\dots,b^{(n)})$, die die Bitfolge mit Hilfe von $SC$ aus $K$ extrahiert:
\begin{align*}
	&K^{(0)} \coloneqq K\\
	&\textnormal{Für } i = 0,\dots,n-1\colon (b^{(i+1)},K^{(i+1)}) \coloneqq SC(K^{(i)}) 
\end{align*}
$G$ bezeichnen wir auch als Generator. Für das Chiffrat $\ciphert$ gilt $\ciphert \coloneqq \plaint \circ G(K)$, wobei $\circ$
eine binäre Verknüpfung auf Bits ist. Häufig wird hier die logische XOR-Operation, das heißt, die Verknüpfung der Gruppe
$\mathbbm{Z}_{2}$, die wir nachfolgend durch $\oplus$ ausdrücken, verwendet.
