...
Für einen Bitstrom $B = 1010$ bezeichnet $B[0] = 0$ somit das am weitesten rechts stehende Bit.  
Ein Beispiel einer solchen linearen Abhängigkeit ist
\begin{align*}
	\plaint[1, 7] \oplus \ciphert[3, 8] = \key[3, 17]\, .
\end{align*}

Die Größe $\left|{p-\frac{1}{2}}\right|$ gibt die Effektivität der Gleichung~\ref{approx} an. Wenn eine effektive lineare Approximation bekannt ist, kann mit der naiven Maximum-Likelihood-Methode ein Schlüsselbit $\key[k_1,\dots, k_c]$ erraten werden. %TODO:(naive) Maximum-Likelihood-Methode verstehen und hier citen.
...
\emph{Bei S-Box $S_5$ gilt, dass das fünfte Eingabebit (von rechts mit eins beginnend gezählt) in $12$ der $64$ möglichen Eingaben mit dem XOR der vier Ausgabebits übereinstimmt.}
...
\begin{minipage}[h]{.5\linewidth}
	...
	\begin{tabular}{>{$}r<{$}@{}>{\tiny $}r<{$}*{5}{>{$}r<{$}}}
		\multicolumn{7}{c}{Expansion $E$}\\
		\hline
		\hline
		31&_{47}&0&1&2&3&4\\
		3&_{41}&4&5&6&7&8\\
		7&_{35}&8&9&10&11&12\\
		11&_{29}&12&13&14&15&16\\
		15&_{23}&16&17&18&19&20\\
		19&_{17}&20&21&22&23&24\\
		23&_{11}&24&25&26&27&28\\
		27&_{5}&28&29&30&31&0
	\end{tabular}
\end{minipage}

Die Tabellen sind zeilenweise zu lesen, so dass das durch den Zellenwert gegebene Bit auf das durch den Zellenindex, der in der ersten Tabellenspalte jeweils beispielhaft notiert ist, gegebene Bit abgebildet wird. Beispielsweise permutiert $P$ das achte Bit des Eingabestrings an die letzte Position des Ausgabestrings, das heißt $F(R^{(i)}, \key^{(i)})[31]$ und das siebzehnte Bit an die erste Stelle, das heißt $F(R^{(i)}, \key^{(i)})[0]$\footnote{Die Tabellen sind dem NIST-Standard \cite{NIST_DES99} entnommen und an die Schreibweise der Matsui-Veröffentlichung \cite{Matsui1994} angepasst.}. 

%Vorher war von "$8$. Bit des Eingabestrings an die erste Position des Ausgabestrings, das heißt $F(R^{(i)}, \key^{(i)})[0]$ und das $17$. Bit an die letzte Stelle, das heißt $F(R^{(i)}, \key^{(i)})[31]$" die Rede
...
Da die Gleichung~\ref{s5relation} die beste Approximation der $F$-Funktion ist, ist damit die Gleichung~\ref{3roundsapprox} die beste Approximation für den $3$-Runden-DES. Um $\key^{(1)}[22] \oplus \key^{(3)}[22]$ zu erhalten, löst man Gleichung~\ref{3roundsapprox} statistisch mit der Maximum-Likelihood-Methode.
