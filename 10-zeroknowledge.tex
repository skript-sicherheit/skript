...
mit $\skey_P = x$ und $\pkey_P = g^x$). Wir verlangen also, dass V während der Kommunikation mit P nichts über $\skey_P$ lernt, was er nicht schon aus $\pkey_P$
berechnen kann.
...
\begin{align*}
\Pr \lbrack \A(1^k,x) = 1 \vert x \leftarrow X \rbrack - \Pr \lbrack \A(1^k, x) = 1 \vert y \leftarrow Y \rbrack
\end{align*}
...
In der Literatur existieren verschiedene Konstruktionen für solche Commitment-Verfahren. Ein bekanntes Beispiel sind Pedersen-Commitments \cite{PedersenCommitments}.
...
Das Entscheidungsproblem, ob ein gegebener Graph dreifärbbar ist, ist NP-vollständig \cite{Graph3ColorIsNpComplete}.
